// components/number-roller.tsx v3.1 — SSR-safe & 稳定 key
"use client";
...
function CharacterSlot({
  ...
  // 使用拒绝采样随机数字用于动画显示
  const randomDigit = React.useCallback(() => secureRandomInt(10), []);
  ...
  if (!isDigit || shouldReduceMotion) {
    return (
      <div className="flex items-center justify-center min-w-[0.3em] px-0.5">
        <span className={cn("inline-block", textStyles)} aria-hidden={false}>
          {char}
        </span>
      </div>
    );
