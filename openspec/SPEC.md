...
### v5.7.5
- 修复设置容器 UI 问题：抽取时长 Slider 下限由 0 改为 1（与 setter clamp 下限一致，消除拖到 0 弹回 1 的跳动）；外观设置主题模式区块在首屏未挂载时渲染 disabled 占位 SelectTrigger（避免空白容器与布局跳动）
- 全仓库版本号统一至 v5.7.5
